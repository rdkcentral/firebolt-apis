...
TEST_F(LifecycleTest, subscribeOnStateChanged2)
{
    ...
    EXPECT_EQ(Firebolt::Error::None, result1.error())
        << "Error: wrong error code returned by first subscribeOnStateChanged";
    ...
    EXPECT_EQ(Firebolt::Error::None, result2.error())
        << "Error: wrong error code returned by second subscribeOnStateChanged";
    ...
    EXPECT_EQ(cbs_called_.count(EXPECTED_CB_ID_1), 0)
        << "Error: callback should not have been called after unsubscribing";
    EXPECT_EQ(cbs_called_.count(EXPECTED_CB_ID_2), 1)
        << "Error: callback 2 should still be called after callback 1 unsubscribed";
}
...
TEST_F(LifecycleTest, subscribeOnStateChanged)
{
    ...
    EXPECT_EQ(std::get<OLD_STATE_INDEX>(cbs_called_[EXPECTED_CB_ID_1]), Firebolt::Lifecycle::LifecycleState::INITIALIZING)
        << "Error: wrong old state in subscription callback";

    EXPECT_EQ(std::get<NEW_STATE_INDEX>(cbs_called_[EXPECTED_CB_ID_1]), Firebolt::Lifecycle::LifecycleState::ACTIVE)
        << "Error: wrong new state in subscription callback";

    EXPECT_EQ(lifecycleImpl_.unsubscribe(BOGUS_SUBSCRIPTION_ID).error(), Firebolt::Error::General)
        << "Error: unsubscribing with bogus subscription id should return General error";

    EXPECT_EQ(lifecycleImpl_.unsubscribe(EXPECTED_CB_ID_1).error(), Firebolt::Error::None)
        << "Error: unsubscribing with valid subscription id should return None error";
