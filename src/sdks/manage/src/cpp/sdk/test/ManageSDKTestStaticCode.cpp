...
namespace FireboltSDK {
    ...
    /* static */ Firebolt::Error ManageTestStaticCode::GetCountryCode()
    {
        ...
        if (status == Firebolt::Error::None) {
            FIREBOLT_LOG_INFO(Logger::Category::Manage, Logger::Module<Tests>(), "CountryCode : %s", response->Value().c_str());
        } else {
            FIREBOLT_LOG_ERROR(Logger::Category::Manage, Logger::Module<Tests>(), "Get %s status = %d\n", method.c_str(), status);
        }
