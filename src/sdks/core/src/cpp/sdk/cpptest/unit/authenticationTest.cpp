...
class AuthenticationTest : public ::testing::Test {
 ...
 TEST_F(AuthenticationTest, Token)
 {
	...
	auto actualValues = Firebolt::IFireboltAccessor::Instance().AuthenticationInterface().token(type, options, &error);

	EXPECT_EQ(error, Firebolt::Error::None);
	EXPECT_EQ(actualValues.value, expectedValues["value"]);
}
