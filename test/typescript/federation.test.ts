...
import { Lifecycle, Discovery } from "../../dist/lib/firebolt";
...
let entityInfoPulled: boolean = false;
let entityInfoReceived: boolean = false;
let entityInfoPushed: boolean = false;
let callbackWiredUp: boolean = false;
let sendCalled: boolean = false;
let correlationId: string;
let secondRegistrationFailed: boolean = false;
  ...
  send: function (message: string) {
    ...
    if (json.method.toLowerCase() === "discovery.onpullentityinfo") {
      ...
      if (correlationId && json.params.correlationId === correlationId) {
        ...
        correlationId &&
        json.params.correlationId === "this-will-fail"
      ) {
        //entityInfoReceived = true
      } else if (
        !json.params.correlationId &&
        json.params.result.entity.identifiers.entityId === "PUSH:345"
      ) {
...
Discovery.entityInfo(dummyData);
